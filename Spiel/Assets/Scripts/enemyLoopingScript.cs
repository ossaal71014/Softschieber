...
public class enemyLoopingScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            {
                transform.Translate((-Vector2.up + new Vector2(0f, istGegenschub)) * Time.deltaTime * 3);
            ...
            {
                jetzt = false;
                if (transform.localPosition.y < 5.5f && transform.localPosition.y > -5.0f)
                {
            ...
            if (rolle && istGegenschub < gegenschub)
            {
                istGegenschub += Time.deltaTime * 1.5f;
            }
        }
        void FixedUpdate()
        {
